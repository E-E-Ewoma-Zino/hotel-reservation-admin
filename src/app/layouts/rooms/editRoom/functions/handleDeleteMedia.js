...
async function handleDeleteMedia(roomId, mediaPath, mediaType, setUpdateCount) {
	console.log("delete", roomId, mediaPath, mediaType);

	const data = { roomId, mediaPath, mediaType };

	try {

		const deletedMedia = await (await deleteRoomMedia(data)).data;

		if (deletedMedia.data) window.swalWithBootstrapButtons.fire({
			icon: 'success',
