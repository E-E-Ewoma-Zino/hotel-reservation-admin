            ...
            <form>
              <Row>
                <Col size="col-sm-4" className="mb-3">
                  <Input
                    name="Name"
                    type="text"
                    placeholder={room.name !== "" ? room.name : "Standard 01"}
                    value={room.name}
                    change={(event) => handleChange(event, setRoom)}
                  />
                </Col>
                <Col size="col-sm-4" className="mb-3">
                  <Input
                    name="Price"
                    type="number"
                    placeholder={room.price !== "" ? room.price : "2500"}
                    value={room.price}
                    change={(event) => handleChange(event, setRoom)}
                  />
                </Col>
                <Col size="col-sm-4" className="mb-3">
                  <Input
                    name="Type"
                    type="text"
                    placeholder={room.type !== "" ? room.type : "Standard"}
                    value={room.type}
                    change={(event) => handleChange(event, setRoom)}
                  />
                </Col>
                ...
                <Col size="col-sm-6" className="mb-3">
                  <Input
                    ...
                    placeholder={
                      room.features !== ""
                        ? room.features
                        : "Eg: TV,Air Conditional,Shampoo"
                    }
                    value={room.features}
                    change={(event) => handleChange(event, setRoom)}
                  />
                  <small className="font-monospace">
                    Separate words using commers
                  </small>
                </Col>
                <Col size="col-sm-6" className="mb-3">
                  <TextArea
              ...
              <Row>
                <Col size="col-sm-6" className="mb-3">
                  <Input
                    ...
                    multiple="multiple"
                    accept="image/*"
                    change={(event) => handleMediaChange(event, setRoom)}
                  />
                </Col>
                <Col size="col-sm-6" className="mb-3">
                  <Input
              ...
              <div className="mt-3">
                <Button
                  ...
                  type="submit"
                  click={(event) =>
                    handleFormSubmit(event, room, roomId, setUpdateCount)
                  }
                />
              </div>
            </form>
        ...
        <Col size="col-12">
          <Card className="card card-body border-0 shadow mb-4">
            {Array.isArray(room.images) && room.images.length ? (
              <h2 className="h5 mb-4">Room Images</h2>
            ) : (
              <></>
            )}
            <Row>
                ...
                room.images.map((img, index) => (
                  <Col key={index} size="col-sm-4" className="mb-3">
                    <div className="position-relative hover-over">
                      ...
                      <DropDownBtn
                        name="Actions"
                        buttonClass="btn btn-sm btn-danger d-inline-flex align-items-center dropdown-toggle"
                        dropdownclass="hide hover-show position-absolute top-0 left-0 transform-12-27"
                      >
                        <DropDownItem
                          name="View"
                          className="disabled"
                        ...
                        <DropDownItem
                          name="Delete"
                          icon={
                            <FireIcon className="dropdown-icon text-danger me-2" />
                          }
                          click={() =>
                            handleDeleteMedia(
                              roomId,
            ...
            {Array.isArray(room.videos) && room.videos.length ? (
              <h2 className="h5 mb-4">Room Video</h2>
            ) : (
              <></>
            )}
            <Row>
              {Array.isArray(room.videos) &&
